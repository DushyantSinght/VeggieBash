...
const onSubmitHandler = async (event) => {
  ...
  return (
    <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
            <form  onSubmit={onSubmitHandler} key={files.length} className="md:p-10 p-4 space-y-5 max-w-lg">
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            ...
                            <input onChange={(e)=>{const updatedFiles = [...files];
                                updatedFiles[index] = e.target.files[0] ;setFiles(updatedFiles)
                            }}
                ...
                <button className="px-8 py-2.5 green text-white font-medium rounded cursor-pointer">ADD</button>
            </form>
        </div>
  )
}
