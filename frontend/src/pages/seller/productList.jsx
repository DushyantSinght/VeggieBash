...
const ProductList = () => {
    const {products,axios, fetchProducts, setProducts} = useAppcontext();
const toggleStock = async (id, newStock) => {
  try {
    const { data } = await axios.post("http://localhost:4000/api/product/stock", {
      id,
      inStock: newStock,
    });

    if (data.success) {
      // ✅ replace only updated product
      setProducts((prev) =>
                                ...
                                <tr key={product._id} className="border-t border-gray-500/20">
                                    ...
                                    <td className="px-4 py-3">
                                        <label className="relative inline-flex items-center cursor-pointer text-gray-900 gap-3">
                                            ...
                                            <input type="checkbox"
                                            checked={!!product.inStock}   // make sure it’s boolean
