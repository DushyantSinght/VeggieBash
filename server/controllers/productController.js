...
export const addProduct = async (req, res) => {
  try {
    ...
    // Upload files directly from memory to Cloudinary
    const imagesUrl = await Promise.all(
      images.map(
        (item) =>
          new Promise((resolve, reject) => {
            ...
            uploadStream.end(item.buffer); // use buffer instead of file path
          })
      )
    );

    // Save product in MongoDB with Cloudinary URLs
